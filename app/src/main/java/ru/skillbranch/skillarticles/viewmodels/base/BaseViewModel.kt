...
abstract class BaseViewModel<T : IViewModelState>(
    ...
    protected fun launchSafety(
        ...
        val errHandlerLocal = CoroutineExceptionHandler { _, err ->
            errHandler?.invoke(err) ?: when (err) {
                is NoNetworkError -> notify(Notify.TextMessage("Network not available, check internet connectionт"))
        ...
        (viewModelScope + errHandlerLocal).launch {
            showLoading()
            block()
        }.invokeOnCompletion {
            hideLoading()
